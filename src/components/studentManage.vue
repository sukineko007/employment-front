    ...
    methods:{
      onSubmit(){  //提交搜索内容
        if ( this.searchContent == "") {
          ...
          this.loading = true;
          this.isSerach = true;
          this.axios.get('http://localhost:8700/student/searchStudent',{
            params: {
      ...
      handleDel(id){   //删除一条记录
        this.$confirm('此操作将永久删除该学生, 是否继续?', '提示',{
          ...
          params.append("id",id);
          this.axios.post('http://localhost:8700/student/delStudent',params)
            .then( res => {
              this.$message.success('删除成功！');
              //删除成功后重新请求当前页
              this.axios.get('http://localhost:8700/student/getStudent',{
                params: {
                }
              })
                .then( res => {
      ...
      batchDel () {  //批量删除
        ...
        if (count == 0) {
          ...
          this.$confirm('此操作将永久删除这些学生, 是否继续?', '提示',{
            ...
            this.loading = true;
            this.axios.post('http://localhost:8700/student/bachDelete',params)
              .then( res => {
                this.$message.success('删除成功！');
                //删除成功后其重新请求当前页
                this.axios.get('http://localhost:8700/student/getStudent',{
