    ...
    methods:{
      onSubmit(){  //提交搜索内容
        if ( this.searchContent == "") {
          ...
          this.loading = true;
          this.isSerach = true;
          this.axios.get('http://localhost:8700/wanted/searchWanted',{
            params: {
      ...
      handleDel(id){   //删除一条记录
        this.$confirm('此操作将永久删除该信息, 是否继续?', '提示',{
          ...
          params.append("id",id);
          this.axios.post('http://localhost:8700/wanted/delWanted',params)
            .then( res => {
              this.$message.success('删除成功！');
              //删除成功后重新请求当前页
              this.axios.get('http://localhost:8700/wanted/getWanted',{
                params: {
                }
              })
                .then( res => {
      ...
      handleCurrentChange(val) {  //更改当前页,重新发起请求
        ...
        if (this.isSerach) {   //如果是搜索内容页面切换
          ...
          if(this.userEntity.type != 2){
            ...
            this.axios.get('http://localhost:8700/wanted/getWanted2',{
              params: {
