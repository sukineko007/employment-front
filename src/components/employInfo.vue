<template>
  <div>
    <div class="myaction">
      <div class="first">
        <div class="left">
          <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="搜索就业信息" class="myelform">
              <el-input v-model="searchContent" placeholder="请输入学生学号"></el-input>
            </el-form-item>
      ...
      <div class="bottom">
        <div class="left">
          <el-button icon="el-icon-plus" type="primary" @click="addUser">添加就业信息</el-button>
        </div>
    ...
    methods:{
      onSubmit(){  //提交搜索内容
        if ( this.searchContent == "") {
          ...
          this.loading = true;
          this.isSerach = true;
          this.axios.get('http://localhost:8700/employ/searchEmploy',{
            params: {
      ...
      handleEdit(param){   //点击编辑时向editinfo发送消息
        bus.$emit("employEdit", {flag:true,value:param});
      },
      handleDel(id){   //删除一条记录
        this.$confirm('此操作将永久删除该信息, 是否继续?', '提示',{
          ...
          params.append("id",id);
          this.axios.post('http://localhost:8700/employ/delEmploy',params)
            .then( res => {
              this.$message.success('删除成功！');
              //删除成功后重新请求当前页
              this.axios.get('http://localhost:8700/employ/getEmploy',{
                params: {
                }
              })
                .then( res => {
      ...
      addUser() {  //添加一条题目
        if ( this.currentPage == Math.ceil( this.count / this.currentLimit) ) //判断是否是最后一页
          this.isLast = true;
        bus.$emit("employAdd", {flag:true,type:this.region,isLast:this.isLast});
      },
      ...
      handleCurrentChange(val) {  //更改当前页,重新发起请求
        ...
        if (this.isSerach) {   //如果是搜索内容页面切换
          this.axios.get('http://localhost:8700/recruit/searchRecruit',{
            params: {
              data:this.searchContent
            }
          })
      ...
      batchDel () {  //批量删除
        ...
        if (count == 0) {
          ...
          this.$confirm('此操作将永久删除这些招聘信息？, 是否继续?', '提示',{
            ...
            this.loading = true;
            this.axios.post('http://localhost:8700/recruit/bachDelete',params)
              .then( res => {
                this.$message.success('删除成功！');
                //删除成功后其重新请求当前页
                this.axios.get('http://localhost:8700/recruit/getRecruit',{
                  params: {
                  }
                })
                  .then( res => {
